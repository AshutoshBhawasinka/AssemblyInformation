...
Echo Building Assembly Information Shell Extension
msbuild /p:configuration=Release /p:platform=Win32 AssemblyInformationShellExt.sln>%TEMP_BUILD_LOG%
if errorlevel 1 (
	echo Failed to build AssemblyInformationShellExt in Win32 Release Mode.
	type %TEMP_BUILD_LOG%
	if not defined NOPAUSE pause
	exit /b 1
)
msbuild /p:configuration=Release /p:platform=x64 AssemblyInformationShellExt.sln>%TEMP_BUILD_LOG%
if errorlevel 1 (
	echo Failed to build AssemblyInformationShellExt inx64 Release Mode.
	type %TEMP_BUILD_LOG%
	if not defined NOPAUSE pause
	exit /b 1
)
...
del AssemblyInformationShellExt.sdf>nul 2>nul

del %TEMP_BUILD_LOG%>nul 2>nul
rd /S /Q obj>nul 2>nul
rd /S /Q ipch>nul 2>nul
Echo Completed Building Assembly Information Shell Extension
if not defined NOPAUSE pause
